...
//! - (D)TLS using pre-shared keys (PSK): The easiest method, uses a symmetric/pre-shared key to
...
//!   Constrained RESTful Environments (OSCORE, RFC 8613) to encrypt messages on the application
//!   layer.
//!
//! # Configuration
//!
//! Logically, `libcoap` provides two different structures for DTLS configuration: One for PSK
//! configuration and another one for both PKI and RPK configuration.
...
//! [`CoapContext`](crate::CoapContext) using for server-side use.
//!
