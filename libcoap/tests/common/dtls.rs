...
use libcoap_rs::crypto::pki_rpk::{KeyDef, KeyType, NonCertVerifying, PkiRpkContext, PkiRpkContextBuilder, ServerPkiRpkCryptoContext};
...
#[allow(unused)]
pub fn dtls_client_server_request_common<KTY: KeyType, FC, FS>(client_key: impl KeyDef<KeyType=KTY> + 'static, server_key: impl KeyDef<KeyType=KTY> + 'static + Send, client_ctx_setup: FC, server_ctx_setup: FS)
where
    ...
    ClientCryptoContext<'static>: From<PkiRpkContext<'static, KTY>>,
{
    ...
    let server_handle = common::spawn_test_server(move |mut context: CoapContext| {
        let server_crypto_ctx = server_ctx_setup(PkiRpkContextBuilder::<'static, KTY, NonCertVerifying>::new(server_key));
