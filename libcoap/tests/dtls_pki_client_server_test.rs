...
use libcoap_rs::crypto::pki_rpk::{Asn1PrivateKeyType, NonCertVerifying, PkiRpkContextBuilder};
...
pub fn dtls_pki_pem_file_client_server_request() {
    ...
    let client_key = PkiKeyDef::with_pem_files(key_storage.join("./ca/ca.crt.pem"), key_storage.join("./client/client.crt.pem"), key_storage.join("./client/client.key.pem"));
    let server_key = PkiKeyDef::with_pem_files(key_storage.join("./ca/ca.crt.pem"), key_storage.join("./server/server.crt.pem"), key_storage.join("./server/server.key.pem"));

    let ctx_configurator = |ctx: PkiRpkContextBuilder<'static, Pki, NonCertVerifying>| { ctx.verify_peer_cert().check_common_ca(true).build() };
...
pub fn dtls_pki_pem_memory_client_server_request() {
    ...
    let client_key = PkiKeyDef::with_pem_memory(Vec::from(PEM_CA_CERT), Vec::from(PEM_CLIENT_PUBLIC_CERT), Vec::from(PEM_CLIENT_PRIVATE_KEY));
    let server_key = PkiKeyDef::with_pem_memory(Vec::from(PEM_CA_CERT), Vec::from(PEM_SERVER_PUBLIC_CERT), Vec::from(PEM_SERVER_PRIVATE_KEY));
...
pub fn dtls_pki_asn1_file_client_server_request() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let key_storage = manifest_dir.join("./resources/test-keys");
    let client_key = PkiKeyDef::with_asn1_files(key_storage.join("./ca/ca.crt.der"), key_storage.join("./client/client.crt.der"), key_storage.join("./client/client.key.der"), Asn1PrivateKeyType::Ec);
    let server_key = PkiKeyDef::with_asn1_files(key_storage.join("./ca/ca.crt.der"), key_storage.join("./server/server.crt.der"), key_storage.join("./server/server.key.der"), Asn1PrivateKeyType::Ec);
...
pub fn dtls_pki_asn1_memory_client_server_request() {
    ...
    let client_key = PkiKeyDef::with_asn1_memory(Vec::from(DER_CA_CERT), Vec::from(DER_CLIENT_PUBLIC_CERT), Vec::from(DER_CLIENT_PRIVATE_KEY), Asn1PrivateKeyType::Ec);
    let server_key = PkiKeyDef::with_asn1_memory(Vec::from(DER_CA_CERT), Vec::from(DER_SERVER_PUBLIC_CERT), Vec::from(DER_SERVER_PRIVATE_KEY), Asn1PrivateKeyType::Ec);
