...
pub fn dtls_pki_pem_memory_client_server_request() {
    const PEM_CLIENT_PUBLIC_KEY: &str = include_str!("../resources/test-keys/client/client.key.pem");
    const PEM_SERVER_PUBLIC_KEY: &str = include_str!("../resources/test-keys/server/server.key.pem");
